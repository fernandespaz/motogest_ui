...
import type { ContaPagarResponse } from '@/api/types';
...
export function ContasPagarTab() {
  ...
  const [cancelando, setCancelando] = useState<ContaPagarResponse | null>(null);

  const { data, isLoading } = useContasPagar({ page, size: 20 });
  ...
  async function handlePagar(id: number) {
    try {
      ...
      toast.error(extractErrorMessage(error, 'Não foi possível marcar como paga.'));
    }
  }
      ...
      <Card>
        <DataTable<ContaPagarResponse>
          ...
          columns={[
            ...
            { header: 'Fornecedor', render: (row) => row.fornecedor || '—', hideBelow: 'sm' },
